    ...
    <div className="isolate px-6 py-24 sm:py-32 lg:px-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {data.data.map((item: { id: string; name: string; desc: string }) => (
          <div
            key={item.id}
            className="bg-gradient-to-br h-[8rem] cursor-pointer from-[#6A1E55] to-[#3B1C32] shadow-lg rounded-md border-2 border-white shadow-indigo-500/50 p-4 flex flex-col justify-start items-start"
          >
        ...
        <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
          <BoardForm mutate={createNewBoard} id={''} name={""} desc={''} />
        </Modal>
      </div>
    </div>
  );
